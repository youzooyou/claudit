...
| `webRequest` | **Sensitive** | Capturing API requests made by the target page for test verification | DevTools Protocol (used in parallel, not a replacement) |
...
| `<all_urls>` host | **Sensitive** | Users run tests against arbitrary admin URLs; domains are user-selected at runtime and cannot be enumerated | `optional_host_permissions` (considered for future release) |
...
**Use:** Reading the URL and title of tabs other than the active one for multi-tab coordination (e.g., sidebar state per tab), and detecting tab removal to clean up session state.

**Why needed:** Claudit maintains per-tab session state (chat history, agent state) keyed by tab ID. When a user closes a tab, the extension must archive or discard the associated session, which requires the `chrome.tabs.onRemoved` listener.
...
**Use:** Captures HTTP requests made by the target admin page for use in test verification (e.g., confirming that clicking "Save" triggered the expected `POST /admin/items` call).

**Why needed:** E2E tests of web admins frequently need to assert that UI actions produced the correct API calls. `chrome.webRequest` with the `requestBody` optional parameter is the only way to capture request payloads without requiring the target page to opt in.

**Data accessed:** URL, method, request body, status, and timing of HTTP requests from the active tab. Capture is opt-in per test session (started by `browser_network_start`, stopped by session end). Requests are kept in a 500-entry ring buffer in `chrome.storage.session` only; they are never sent anywhere except to Claude when the user explicitly retrieves them via `browser_network_get` during a chat.

**Why sensitive:** `webRequest` + `requestBody` can observe form submissions. Claudit uses this only for the user's own admin panel, under the user's explicit test session.
...
- Network request body inspection (as a second channel beside `webRequest`)
...
**Important:** Chrome shows a persistent yellow "Claudit started debugging this browser" banner whenever the debugger is attached. Users always know when CDP is active. Claudit detaches immediately after each tool call via a `finally` block to minimize banner duration.

**Data accessed:** Only the tab the user is testing, only during explicit tool calls.

**Why sensitive:** `debugger` is the most powerful Chrome extension API. Claudit uses it only for direct input dispatch and screenshots, and only within the scope of a user-initiated test. The extension's source code is published and auditable.
...
**Use:** Allows content-script injection, CDP attachment, and `webRequest` capture on any URL the user visits.
...
- **`optional_host_permissions`** — Users would have to grant permission per-domain via a prompt on first test. Considered for a future release, but rejected for v2.0 because it significantly disrupts the "open side panel and start testing" flow that is Claudit's primary UX goal.
- **Site enumeration via Options page** — Considered, but rejected because QA workflows often involve ephemeral staging URLs that change daily.

**Why sensitive:** `<all_urls>` grants the most powerful host access Chrome offers. Claudit mitigates risk by:
1. Never activating automation without an explicit user chat message
2. Clear visual indicators (side panel open, debugger banner) whenever the extension is acting on a page
3. All code is published and auditable
4. No data is sent anywhere except the Claude API endpoint the user configured in Options (under their credentials) and the user's own selected folders
...
> Required to persist the user's Claude API credentials (API key or bearer token), the user-configurable API Base URL, project configuration (spec folder, data folder), and test result history. Service Workers cannot use localStorage, so chrome.storage.local is the only option.
...
> Required to read URL and title of tabs, and to clean up per-tab session state when tabs close. Claudit maintains separate chat history per tab.

### `webRequest` justification

> Required to capture HTTP requests made by the admin page for test verification. E2E tests frequently assert that UI actions trigger expected API calls. Capture is opt-in per session, stored only in session storage, and never transmitted except when the user explicitly sends it to Claude via a chat.
...
> Claudit is a QA testing tool for arbitrary web admin panels. Target URLs are not known in advance — QA engineers test different staging and production environments that change over time. A narrower permission would require per-domain prompts on every new test target, disrupting the core workflow. All automation requires explicit user initiation via a chat message, and Chrome's debugger banner makes automation visible to the user.
...
5. Test the build with `npm run pack` and audit the resulting zip
